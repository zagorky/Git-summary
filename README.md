...
`git commit --amend -m “сообщение”` -  обновить предыдущий коммит, можно изменить сообщение последнего коммита и/или контент, работает до отправки коммита в удаленный репозиторий.
...
`git branch -d имяВетки` - удаляет выбранную ветку; работает с локальными ветками (только если они слиты с другими).
...
1. Исправить конфликт вручную: конфликтующие строки контента выделены метками: начало конфликтующих строк <<<<<<<, разделение двух блоков конфликтующих строк ======= и конец конфликтующих строк >>>>>>> -> оставить нужный контент, удалив метки -> git add -> сделать коммит
