...
Имена пользователя и адрес электронной почты, используемые для идентификации коммитов, задаются с помощью команды `git config user.name` и `git config user.email`.
...
`git config --global core.editor 'путь до файла IDE.exe'` - установка по умолчанию редактора кода

`git config --global alias.last 'log -1 HEAD'` - создать алиасы и собственные команды (log -1 HEAD)
...
`git clone url-репозитория` - клонирование удаленного репозитория на компьютер

`master` — это основная ветка в локальном репозитории, а `origin/master` — это версия основной ветки на удалённом репозитории (обычно именуемом `origin`).
...
`git diff` -выводит различия по внесенным изменениям в непроиндексированных файлах

`git add имяФайла` - добавляет файл в индекс и позволяет начинать их отслеживать
  
`git diff —-staged` -показывает различия между проиндексированным файлом и последними изменениями
  
`git reset имяФайла` - удаляет файл из индекса
...
`git rm —-cached имяФайла` - удаляет выбранный файл из контроля версий, его изменения не отслеживаются, но он останется на месте
  
`git mv имяФайла новоеИмяФайла` - перемещает и переименовывает выбранный файл, занося в индекс
  
### Временное сохранение
  
`git stash` или `git stash save 'описание'` - временно сохраняет непроиндексированные измененные файлы
...
`git log` - история комиков для текущей ветки и вся информация о них, последний коммит вверху
...
`git blame имяФайла` - посмотреть кто редактировал файл 

`git bisect` - ищет определенный коммит бинарным поиском
...
![[Pasted image 20241103152948.png]]
...
![[Pasted image 20241103152908.png]]
...
![[Pasted image 20241103165017.png]]
...
![[Pasted image 20241103155957.png]]
![[Pasted image 20241103160046.png]]
...
![[Pasted image 20241103153414.png]]
